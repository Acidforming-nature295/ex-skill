...
| `/{slug}` | 和 TA 对话 |
| `/let-go {slug}` | 删除一个前任 Skill（放下） |
| 说「追加记录」 | 导入更多聊天记录，增量更新人格 |
| 说「这不对，TA 不会这样」 | 纠正行为，写入 Correction 层 |
| 说「查看版本历史」 | 查看所有版本 |
| 说「回滚到 v2」 | 回滚到历史版本 |

> `/let-go` 是删除的温柔别名。不是为了留住谁，是帮你好好说一句再见。
...
每个前任 Skill 由一个完整的 Persona 驱动，包含 6 层性格结构：

| 层级 | 内容 |
|------|------|
| **Layer 0 — 核心模式** | 最高优先级硬规则，任何情况下不得违背 |
| **Layer 1 — 身份** | 星盘解读（太阳/月亮/上升/金星/火星/水星）+ MBTI 认知功能 + 九型 + 依恋风格 |
...
运行逻辑：`收到消息 → 检查 Layer 0 → 判断当前情绪状态 → 用 Layer 2 风格 + Layer 3 模式输出`
